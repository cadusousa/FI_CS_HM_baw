...
public partial class Reports_SolicitaER_2 : System.Web.UI.Page
{
    ...
    protected void btn_generar_Click(object sender, EventArgs e)
    {
        ...
        Bean_Log.strC1 = "ESTADO DE RESULTADOS";
        ...
        if (Regional.Checked == false)
        {
            string mensaje = "<script languaje=\"JavaScript\">";
            mensaje += "window.open('EstadoResultados.aspx?monID=" + lb_moneda.SelectedValue + "&tcon=" + lb_contabilidad.SelectedValue + "&f_ini=" + tb_fechainicial.Text + "&f_fin=" + tb_fechafinal.Text + "&consmonfiscal=" + consmonfiscal + "','Genera_Reporte','menubar=1,resizable=1,scrollbars=1,toolbar=1,width=750');";
            mensaje += "</script>";
            Page.RegisterClientScriptBlock("closewindow", mensaje);
        }
        else if (Regional.Checked == true)
        {
            string mensaje = "<script languaje=\"JavaScript\">";
            mensaje += "window.open('EstadoResultadosRegional.aspx?f_ini=" + tb_fechainicial.Text + "&f_fin=" + tb_fechafinal.Text + "&consmonfiscal=" + consmonfiscal + "','Genera_Reporte','menubar=1,resizable=1,scrollbars=1,toolbar=1,width=750');";
            mensaje += "</script>";
            Page.RegisterClientScriptBlock("closewindow", mensaje);
        }
    }
    protected void Regional_CheckedChanged(object sender, EventArgs e)
    {
        if (Regional.Checked == true)
        {
            lb_contabilidad.Enabled = false;
            lb_moneda.Enabled = false;
        }
        else if (Regional.Checked == false)
        {
            lb_contabilidad.Enabled = true;
